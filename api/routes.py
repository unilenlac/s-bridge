...
# ---------------------------------------------------------------------------
# Direct Collate endpoint — collate a JSON payload directly via CollateX
# ---------------------------------------------------------------------------
...
@router.post(
    "/collate",
    description=(
        "Direct CollateX Collation. Accepts a pre-built CollateX JSON payload "
        "(such as the payload generated by s-bridge's /prepare endpoint or a standard CollateX JSON payload) "
        "and executes collation directly via CollateX. Returns the aligned result directly without persisting to DB or Stemmarest."
    ),
)
async def collate(
    *,
    req: Union[CollatexResponse, Dict[str, Any]],
    ...
    try:
        if isinstance(req, CollatexResponse):
        ...
        if "witnesses" not in payload:
            raise HTTPException(
                status_code=400,
                detail="Invalid CollateX payload: 'witnesses' field is required.",
            )
        ...
        headers = {
            "X-Collatex-Algorithm": used_algo,
            "X-Collatex-Fallback": str(fell_back).lower(),
        }

        if isinstance(result, str):
            return Response(content=result, media_type=output_format, headers=headers)
        ...
        raise
    except CollatexError as e:
        logger.error(f"CollateX error during direct collation: {e}", exc_info=True)
        raise HTTPException(status_code=502, detail=str(e))
    except Exception as e:
        logger.error(f"Error during direct collation: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
