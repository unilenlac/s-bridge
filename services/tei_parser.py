...
class TEIParser:
    def __init__(
        self,
        abbr_file: Optional[str] = None,
        custom_tags: Optional[Dict[str, Any]] = None,
    ):
        self.custom_tags: Dict[str, Any] = (
    ...
    def _get_rightmost_text_node(
        self, parent: ET.Element, child_index: int
        ...
        for parent in element.iter():
            for child_index in range(len(parent) - 1, -1, -1):
                ...
                if tag_name in ["lb", "pb"]:
                    ...
                    node_with_text, attr_name = self._get_rightmost_text_node(
                        parent, child_index
                    )
                    ...
                    if child_index > 0:
                        ...
                        if is_hyphenated or is_no_break:
                            if is_hyphenated:
                                ...
                                setattr(
                                    node_with_text, attr_name, prior_text.rstrip()[:-1]
                                )
                            else:
                            ...
                            prev_sibling.tail = (prev_sibling.tail or "") + (
                                child.tail or ""
                            ).lstrip()
                        else:
                            # Not hyphenated and break != 'no'. Attach with a space
                            prev_sibling.tail = (
                                (prev_sibling.tail or "") + " " + (child.tail or "")
                            )
                    else:
                        ...
                        if is_hyphenated or is_no_break:
                            if is_hyphenated:
                                ...
                                parent.text = (
                                    prior_text.rstrip() + (child.tail or "").lstrip()
                                )
                        else:
                    ...
                    if rest:
                        ...
                        if rest_meta.get("abbr"):
                            ...
                            if raw_abbr_rest in self.abbr_dict:
                                rest = rest.replace(
                                    raw_abbr_rest, self.abbr_dict[raw_abbr_rest]
                                )
    ...
    def _extract_text_with_metadata(
        ...
        if element.text:
            self._process_text_chunk(
                element.text, metadata_stack, pending_metadata, results
            )

        # 3. Phase 2: Recurse Children
        for child in element:
            self._extract_text_with_metadata(
                child, metadata_stack, pending_metadata, results
            )
        ...
        if element.tail:
            self._process_text_chunk(
                element.tail, metadata_stack, pending_metadata, results
            )

        return results, pending_metadata

    def _build_normalized_metadata_map(
        self, text_segments: List[TextSegment]
    ) -> Tuple[str, MetadataMap]:
        """
    ...
    def get_metadata_for_token(
        self, char_start: int, char_stop: int, metadata_map: MetadataMap
    ) -> Dict[str, Any]:
